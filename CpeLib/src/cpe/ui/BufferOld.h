...
public:
    ...
    BufferOld &pushBack(const std::string &str);

    BufferOld &pushBack(const BufferOld &buffer);
    ...
    BufferOld &operator<<(const std::string &str);

    BufferOld &operator<<(BufferOld &(*manip)(BufferOld &));

    BufferOld &operator<<(const BufferOld &buf);
...
private:
    ...
    struct _Symbol {
        ...
        explicit _Symbol(const BufferOld *buffer) : mBuffer(buffer),
        ...
        int mType = UNDEFINED;
        union {
            const BufferOld *mBuffer = nullptr;
            _StyledChar mSChar;
        };
    };
